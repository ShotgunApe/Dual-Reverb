...
\title{An Analysis of Artificial Reverberation Algorithms}
...
\begin{abstract}
Artificial reverberation is a method of adding ``echo'' to a particular sound that a musician uses to achieve a desired effect. In doing so, one is able to take a particular sound with a short response and simulate the timbre it would make should it be played in a bathroom, concert hall, or cathedral. Common C++ implementations of this effect usually fall under two different categories: \textit{Feedback Delay Networks} (FDNs) or \textit{Convolution Algorithms}. The former creates the effect via a number of delay lines in parallel, resulting in a reverberated sound provided a sufficiently high echo density is created. The latter []. This thesis studies the practical and sonic differences between these algorithms and their implementations. The final program allows producers to use [].
\end{abstract}
