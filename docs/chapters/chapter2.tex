...
If a buffer is dropped in the digital system, the data cannot be output by the speaker. The resulting sound will create ``clicks'' and ``pops'' where there is a gap in the audio stream \cite{Walker_2005}.
...
The delivery of each chunk of data is dependent of the implementation of the audio driver. For example, one such implementation may utilize a double buffer to help prevent unfinished data being read [asio sdk]. Regardless of implementation, processing must be done by the time the sound card iterrupts the processor for a new buffer in the continuous stream of audio.

Imagine a bus. At regular intervals this bus arrives to take passengers to their destination. In the time that bus takes its passengers to their destination and returns, the next bus must be ready to leave. Should anyone not be in the bus at this point, it will leave without them. It can still drive, but at the cost of missing passengers.
...
Audio is first processed by the audio plugin under a particular host application. This host application is most commonly a \textit{Digital Audio Workstation} (DAW), which handles several different audio plugins to generate and manipulate sound. This audio is then processed by the audio driver into buffers. These buffers are sent to the integrated sound card via the OS's drivers, [which is output to the system's speakers via the integrated sound card or audio chipset] bleh. Output is completed by the Digital-Audio Converter (DAC) which converts the discrete signal into a continuous form. (fix to prevent myself from being repetitive)

A real-time program does not simply play audio files akin to one using sound effects for a video game; the program needs to play this sound as it is being generated on-the-fly. As one can imagine, this will provide limitations into the implementation of the program to prevent buffers from being dropped.
