...
\begin{figure}[h] % [h] used to prevent {figure} from doing weird positioning
	...
	\begin{center}
		\fbox{
		\begin{tikzpicture}
			\node (start) [inner sep=10pt, startstop] {Controller / \\ Data Model};
			\node (A) [startstop, right of=start, xshift=5cm] {View};
			...
			\draw [->, thick] (start) ++(-1.5,0.5) -- +(-2,0) -- +(-2,-1) -- node[anchor=south] {\footnotesize{Change}} +(0, -1) (start);
		\end{tikzpicture}
		}
		\caption{A Combined Controller and Data Model \cite{Bucanek2009}.}
	\end{center}
\end{figure}
