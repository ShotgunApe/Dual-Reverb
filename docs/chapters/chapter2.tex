...
The delivery of each chunk of data is dependent of the implementation of the audio driver. For example, one such implementation may utilize a double buffer to help prevent unfinished data being read [asio sdk]. Regardless of implementation, processing must be done by the time the sound card iterrupts the processor for a new buffer in the continuous stream of audio. [] (do i even need this?)
...
Consider the scenario where an unrelated interrupt prevents a buffer from being processed in real-time:

[insert graph about audio data being late]

For whatever reason, if data is not available for the sound card to access, the resulting sound will create ``clicks'' and ``pops'' where there is a gap in the audio stream \cite{Walker_2005}. To summarize, Figure 3.4 describes how this data travels in a digital setting.
...
A real time program does not simply play audio files akin to one using sound effects for a video game; the program needs to play this sound as it is being generated on-the-fly. This provides limitations that prevent one from using ]\verb|std::cout| [to print information to the terminal during execution, for example. (eh)
