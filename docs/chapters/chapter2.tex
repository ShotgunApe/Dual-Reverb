...
The delivery of each chunk of data is dependent of the implementation of the audio driver.


By utilizing a double buffer to prevent unfinished data being read, the program [].

An issue occurs if time spent []. For this reason, processing these buffers need to be labeled a higher priority when interrupting the processor.
