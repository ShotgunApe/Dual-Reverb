...
To process sound digitally, there are a few different methods of doing so: []. Unlike a music player, our program has to follow the set schedule requested by the audio driver of the device. At regular intervals, the sound card of a device requests audio data to be sent to the system's speakers. [find citations]
