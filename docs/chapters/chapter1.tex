...
Reverb can be broadly defined as the persistence of sound after its initial impulse response [find good definition]. This can take the form of sound waves bouncing off the walls of a room, for example. It provides a texture that is considered ``wet'' and washes out the sound of an otherwise clear tone. In small amounts, it can be tasteful and enhance the timbre of an instrument. In large amounts, it can overpower the initial tone and can disrupt other elements of the music, such as rhythm.
...
Nearly every song one hears on the radio utilizes some kind of music application which allows one to record, edit, and manipulate several audio sources and recordings. Such applications are known as \textit{Digital Audio Workstations} (DAWs), one such program that reverb is commonly used in. By allowing the producer to manipulate these recordings in real time, musicians can quickly and efficiently create the music that one hears today. DAWs accomplish this by providing tools to record the notes or sounds of various instruments through multiple means. [Maybe talk about VSTs first - then MIDI? - I planned on doing this originally in the next paragraph] For example, standards such as MIDI exist that allow a musician to connect musical instruments to their computer. MIDI notes recorded in this manner can perform any type of virtual instrument stored on the machine. One such problem that programmers face is the latency of such programs. Should the resulting sound come too late when a key is pressed, then any attempt at recording will feel unnatural and clunky. Discussion of how to prevent these issues are discussed in Chapter 2 [find out how to reference chapters/figures properly].

To manipulate the timbre of a sound further, DAWs allow the producer to apply a number of additional programs to a generator in series through the channel the sound originates from. [talk about generator vsts first? - synths/samplers/etc.]. These effect plugins are
